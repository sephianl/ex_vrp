...
struct SearchRouteData {
    std::shared_ptr<ProblemData> problemData;  // Keep problem data alive
    std::vector<std::unique_ptr<search::Route::Node>> ownedNodes;  // Nodes we own
    std::unique_ptr<search::Route> route;  // Must be last - destroyed first!

    SearchRouteData(std::unique_ptr<search::Route> r, std::shared_ptr<ProblemData> pd)
...
struct SearchRouteResource {
    ...
    SearchRouteResource(std::unique_ptr<search::Route> r, std::shared_ptr<ProblemData> pd)
    ...
    std::vector<std::unique_ptr<search::Route::Node>>& ownedNodes() { return data->ownedNodes; }
};

// Wrap search::Route::Node for resource management
struct SearchNodeResource {
    search::Route::Node* node;  // Pointer to node (may be owned by route or by us)
    bool owned;  // Whether we own this node
    std::shared_ptr<ProblemData> problemData;  // Keep problem data alive
    std::shared_ptr<SearchRouteData> parentRoute;  // Keep parent route alive (if node is from a route)

    // Constructor for standalone nodes (owned by us)
    SearchNodeResource(search::Route::Node* n, bool o, std::shared_ptr<ProblemData> pd)
        : node(n), owned(o), problemData(std::move(pd)), parentRoute(nullptr) {}

    // Constructor for nodes from a route (keeps route alive)
    SearchNodeResource(search::Route::Node* n, std::shared_ptr<SearchRouteData> parent)
        : node(n), owned(false), problemData(parent->problemData), parentRoute(std::move(parent)) {}
...
struct ExchangeOperatorResource {
    std::unique_ptr<search::Exchange<N, M>> op;
    std::shared_ptr<ProblemData> problemData;

    ExchangeOperatorResource(std::unique_ptr<search::Exchange<N, M>> o, std::shared_ptr<ProblemData> pd)
...
struct SwapStarResource {
    ...
    SwapStarResource(std::unique_ptr<search::SwapStar> o, std::shared_ptr<ProblemData> pd)
        : op(std::move(o)), problemData(std::move(pd)) {}
};
...
struct SwapRoutesResource {
    ...
    SwapRoutesResource(std::unique_ptr<search::SwapRoutes> o, std::shared_ptr<ProblemData> pd)
        : op(std::move(o)), problemData(std::move(pd)) {}
};
...
struct SwapTailsResource {
    ...
    SwapTailsResource(std::unique_ptr<search::SwapTails> o, std::shared_ptr<ProblemData> pd)
        : op(std::move(o)), problemData(std::move(pd)) {}
};
...
struct RelocateWithDepotResource {
    ...
    RelocateWithDepotResource(std::unique_ptr<search::RelocateWithDepot> o, std::shared_ptr<ProblemData> pd)
        : op(std::move(o)), problemData(std::move(pd)) {}
};
...
ProblemData::VehicleType decode_vehicle_type(ErlNifEnv* env, ERL_NIF_TERM term, size_t num_dims) {
    ...
    for (auto l : initial_load_vec) initial_loads.push_back(Load(l));

    // Convert reload_depots to vector<size_t>
    std::vector<size_t> reload_depots;
    for (auto d : reload_depots_vec) reload_depots.push_back(static_cast<size_t>(d));
    ...
    fine::ResourcePtr<SolutionResource> solution_resource)
{
    ...
    return fine::Term(enif_make_list_from_array(env, unassigned.data(), unassigned.size()));
}
 ...
 */
int64_t solution_route_distance(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].distance());
}
 ...
 */
int64_t solution_route_duration(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].duration());
}
 ...
 */
fine::Term solution_route_delivery(
    ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return fine::Term(enif_make_list_from_array(env, terms.data(), terms.size()));
}
 ...
 */
fine::Term solution_route_pickup(
    ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
 ...
 */
int64_t solution_route_time_warp(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].timeWarp());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].excessDistance());
}
 ...
 */
int64_t solution_route_overtime(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].overtime());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].vehicleType());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].startDepot());
}
 ...
 */
int64_t solution_route_end_depot(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].endDepot());
}
 ...
 */
int64_t solution_route_num_trips(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].numTrips());
}
 ...
 */
fine::Term solution_route_centroid(
    ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    if (route_idx < 0 || static_cast<size_t>(route_idx) >= routes.size()) {
        return fine::Term(enif_make_tuple2(env,
 ...
 */
int64_t solution_route_start_time(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].startTime());
}
 ...
 */
int64_t solution_route_end_time(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].endTime());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].serviceDuration());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].travelDuration());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].waitDuration());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].distanceCost());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].durationCost());
}
    ...
    int64_t route_idx)
{
    ...
    return static_cast<int64_t>(routes[static_cast<size_t>(route_idx)].prizes());
}
 ...
 */
fine::Term solution_route_visits(
    ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
 ...
 * Returns a list of tuples: {location, trip, start_service, end_service, wait_duration, time_warp}
 */
fine::Term solution_route_schedule(
    ErlNifEnv* env,
    fine::ResourcePtr<SolutionResource> solution_resource,
    int64_t route_idx)
{
    ...
    for (auto const& visit : schedule) {
        // Create a tuple: {location, trip, start_service, end_service, wait_duration, time_warp}
 ...
 */
int64_t problem_data_num_depots(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
 ...
 */
std::vector<std::tuple<int64_t, int64_t, int64_t, int64_t>> problem_data_clients_nif(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    ...
    for (auto const& client : clients) {
        result.emplace_back(
            static_cast<int64_t>(client.twEarly),
...
std::vector<std::vector<int64_t>> problem_data_distance_matrix_nif(
    ...
    int64_t profile)
{
    auto const& matrix = problem_resource->data->distanceMatrix(static_cast<size_t>(profile));
...
std::vector<std::vector<int64_t>> problem_data_duration_matrix_nif(
    ...
    int64_t profile)
{
    auto const& matrix = problem_resource->data->durationMatrix(static_cast<size_t>(profile));
    size_t n = problem_resource->data->numLocations();
 ...
 */
std::vector<std::tuple<int64_t, int64_t, int64_t>> problem_data_vehicle_types_nif(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    ...
    for (auto const& vt : vehicleTypes) {
        result.emplace_back(
            static_cast<int64_t>(vt.unitDistanceCost),
 ...
 */
std::vector<std::tuple<std::vector<int64_t>, bool>> problem_data_groups_nif(
    [[maybe_unused]] ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    ...
    for (auto const& group : groups) {
        ...
        for (auto client : group.clients()) {
            ...
            clients.push_back(static_cast<int64_t>(client + problem_resource->data->numDepots()));
        }
    ...
    fine::Term opts_term)
{
    ...
    std::vector<std::unique_ptr<pyvrp::search::RelocateWithDepot>> relocate_depot_ops;
    ...
    for (const auto& op_name : node_ops) {
        if (op_name == "exchange10" || op_name == "relocate") {
            exchange10_ops.push_back(std::make_unique<pyvrp::search::Exchange<1, 0>>(problem_data));
            ls.addNodeOperator(*exchange10_ops.back());
        } else if (op_name == "exchange11" || op_name == "swap11") {
            exchange11_ops.push_back(std::make_unique<pyvrp::search::Exchange<1, 1>>(problem_data));
            ls.addNodeOperator(*exchange11_ops.back());
        } else if (op_name == "exchange20" || op_name == "relocate2") {
            exchange20_ops.push_back(std::make_unique<pyvrp::search::Exchange<2, 0>>(problem_data));
            ls.addNodeOperator(*exchange20_ops.back());
        } else if (op_name == "exchange21" || op_name == "swap21") {
            exchange21_ops.push_back(std::make_unique<pyvrp::search::Exchange<2, 1>>(problem_data));
            ls.addNodeOperator(*exchange21_ops.back());
        } else if (op_name == "exchange22" || op_name == "swap22") {
            exchange22_ops.push_back(std::make_unique<pyvrp::search::Exchange<2, 2>>(problem_data));
            ls.addNodeOperator(*exchange22_ops.back());
        } else if (op_name == "exchange30" || op_name == "relocate3") {
            exchange30_ops.push_back(std::make_unique<pyvrp::search::Exchange<3, 0>>(problem_data));
            ls.addNodeOperator(*exchange30_ops.back());
        } else if (op_name == "exchange31" || op_name == "swap31") {
            exchange31_ops.push_back(std::make_unique<pyvrp::search::Exchange<3, 1>>(problem_data));
            ls.addNodeOperator(*exchange31_ops.back());
        } else if (op_name == "exchange32" || op_name == "swap32") {
            exchange32_ops.push_back(std::make_unique<pyvrp::search::Exchange<3, 2>>(problem_data));
            ls.addNodeOperator(*exchange32_ops.back());
        } else if (op_name == "exchange33" || op_name == "swap33") {
            exchange33_ops.push_back(std::make_unique<pyvrp::search::Exchange<3, 3>>(problem_data));
            ls.addNodeOperator(*exchange33_ops.back());
        } else if (op_name == "swap_tails") {
            swap_tails_ops.push_back(std::make_unique<pyvrp::search::SwapTails>(problem_data));
            ls.addNodeOperator(*swap_tails_ops.back());
        } else if (op_name == "relocate_with_depot") {
            relocate_depot_ops.push_back(std::make_unique<pyvrp::search::RelocateWithDepot>(problem_data));
    ...
    for (const auto& op_name : route_ops) {
        if (op_name == "swap_star") {
            swap_star_ops.push_back(std::make_unique<pyvrp::search::SwapStar>(problem_data));
            ls.addRouteOperator(*swap_star_ops.back());
        } else if (op_name == "swap_routes") {
            swap_routes_ops.push_back(std::make_unique<pyvrp::search::SwapRoutes>(problem_data));
    ...
    fine::Term opts_term)
{
    ...
    std::vector<std::pair<std::string, pyvrp::search::NodeOperator*>> node_operator_ptrs;
    std::vector<std::pair<std::string, pyvrp::search::RouteOperator*>> route_operator_ptrs;
    ...
    for (const auto& op_name : node_ops) {
        if (op_name == "exchange10" || op_name == "relocate") {
            exchange10_ops.push_back(std::make_unique<pyvrp::search::Exchange<1, 0>>(problem_data));
            ls.addNodeOperator(*exchange10_ops.back());
            node_operator_ptrs.push_back({op_name, exchange10_ops.back().get()});
        } else if (op_name == "exchange11" || op_name == "swap11") {
            exchange11_ops.push_back(std::make_unique<pyvrp::search::Exchange<1, 1>>(problem_data));
            ls.addNodeOperator(*exchange11_ops.back());
            node_operator_ptrs.push_back({op_name, exchange11_ops.back().get()});
        } else if (op_name == "exchange20" || op_name == "relocate2") {
            exchange20_ops.push_back(std::make_unique<pyvrp::search::Exchange<2, 0>>(problem_data));
            ls.addNodeOperator(*exchange20_ops.back());
            node_operator_ptrs.push_back({op_name, exchange20_ops.back().get()});
        } else if (op_name == "exchange21" || op_name == "swap21") {
            exchange21_ops.push_back(std::make_unique<pyvrp::search::Exchange<2, 1>>(problem_data));
            ls.addNodeOperator(*exchange21_ops.back());
            node_operator_ptrs.push_back({op_name, exchange21_ops.back().get()});
        } else if (op_name == "exchange22" || op_name == "swap22") {
            exchange22_ops.push_back(std::make_unique<pyvrp::search::Exchange<2, 2>>(problem_data));
            ls.addNodeOperator(*exchange22_ops.back());
            node_operator_ptrs.push_back({op_name, exchange22_ops.back().get()});
        } else if (op_name == "exchange30" || op_name == "relocate3") {
            exchange30_ops.push_back(std::make_unique<pyvrp::search::Exchange<3, 0>>(problem_data));
            ls.addNodeOperator(*exchange30_ops.back());
            node_operator_ptrs.push_back({op_name, exchange30_ops.back().get()});
        } else if (op_name == "exchange31" || op_name == "swap31") {
            exchange31_ops.push_back(std::make_unique<pyvrp::search::Exchange<3, 1>>(problem_data));
            ls.addNodeOperator(*exchange31_ops.back());
            node_operator_ptrs.push_back({op_name, exchange31_ops.back().get()});
        } else if (op_name == "exchange32" || op_name == "swap32") {
            exchange32_ops.push_back(std::make_unique<pyvrp::search::Exchange<3, 2>>(problem_data));
            ls.addNodeOperator(*exchange32_ops.back());
            node_operator_ptrs.push_back({op_name, exchange32_ops.back().get()});
        } else if (op_name == "exchange33" || op_name == "swap33") {
            exchange33_ops.push_back(std::make_unique<pyvrp::search::Exchange<3, 3>>(problem_data));
            ls.addNodeOperator(*exchange33_ops.back());
            node_operator_ptrs.push_back({op_name, exchange33_ops.back().get()});
        } else if (op_name == "swap_tails") {
            swap_tails_ops.push_back(std::make_unique<pyvrp::search::SwapTails>(problem_data));
            ls.addNodeOperator(*swap_tails_ops.back());
            node_operator_ptrs.push_back({op_name, swap_tails_ops.back().get()});
        } else if (op_name == "relocate_with_depot") {
            relocate_depot_ops.push_back(std::make_unique<pyvrp::search::RelocateWithDepot>(problem_data));
            ls.addNodeOperator(*relocate_depot_ops.back());
            node_operator_ptrs.push_back({op_name, relocate_depot_ops.back().get()});
        }
    }
    ...
    for (const auto& op_name : route_ops) {
        if (op_name == "swap_star") {
            swap_star_ops.push_back(std::make_unique<pyvrp::search::SwapStar>(problem_data));
            ls.addRouteOperator(*swap_star_ops.back());
            route_operator_ptrs.push_back({op_name, swap_star_ops.back().get()});
        } else if (op_name == "swap_routes") {
            swap_routes_ops.push_back(std::make_unique<pyvrp::search::SwapRoutes>(problem_data));
            ls.addRouteOperator(*swap_routes_ops.back());
            route_operator_ptrs.push_back({op_name, swap_routes_ops.back().get()});
        }
    }
    ...
    enif_make_map_put(env, ls_stats_map,
    ...
    enif_make_map_put(env, result_map,
        ...
        enif_make_list_from_array(env, op_stats_list.data(), op_stats_list.size()),
        &result_map);
...
fine::ResourcePtr<SearchRouteResource> create_search_route_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource,
    int64_t idx,
    int64_t vehicle_type)
{
    ...
    auto route = std::make_unique<search::Route>(
        ...
        static_cast<size_t>(vehicle_type));

    return fine::make_resource<SearchRouteResource>(
        std::move(route),
        problem_resource->data);
}
...
// Get route distance
int64_t search_route_distance_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
...
// Get route duration
int64_t search_route_duration_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
...
// Get route overtime
int64_t search_route_overtime_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
...
std::vector<int64_t> search_route_load_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
...
std::vector<int64_t> search_route_capacity_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
...
// Get centroid
std::tuple<double, double> search_route_centroid_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
    auto const& centroid = route_resource->route()->centroid();
    return {static_cast<double>(centroid.first), static_cast<double>(centroid.second)};
}

FINE_NIF(search_route_centroid_nif, 0);

// Get profile
int64_t search_route_profile_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
...
// Get node at index
fine::ResourcePtr<SearchNodeResource> search_route_get_node_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource,
    int64_t idx)
{
...
fine::Atom search_route_append_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource,
...
fine::Atom search_route_insert_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource,
    int64_t idx,
    fine::ResourcePtr<SearchNodeResource> node_resource)
{
    route_resource->route()->insert(static_cast<size_t>(idx), node_resource->node);
    return fine::Atom("ok");
}
...
fine::Atom search_route_remove_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource,
    int64_t idx)
{
...
// Clear route
fine::Atom search_route_clear_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
...
fine::Atom search_route_update_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource)
{
...
// Static swap nodes
fine::Atom search_route_swap_nif(
    ErlNifEnv* env,
...
bool search_route_overlaps_with_nif(
    ...
    double tolerance)
{
    return route1_resource->route()->overlapsWith(*route2_resource->route(), tolerance);
}
    ...
    int64_t profile)
{
    ...
    return static_cast<int64_t>(
        route.between(static_cast<size_t>(start), static_cast<size_t>(end)).distance(p));
}

FINE_NIF(search_route_dist_between_nif, 0);

// Get distance at specific index
int64_t search_route_dist_at_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchRouteResource> route_resource,
    int64_t idx,
    int64_t profile)
{
    ...
    size_t p = (profile < 0) ? route.profile() : static_cast<size_t>(profile);
    return static_cast<int64_t>(
        route.at(static_cast<size_t>(idx)).distance(p));
}
...
// Create a new Node
fine::ResourcePtr<SearchNodeResource> create_search_node_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource,
    int64_t loc)
{
    auto* node = new search::Route::Node(static_cast<size_t>(loc));
    return fine::make_resource<SearchNodeResource>(
        node,
...
int64_t search_node_client_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchNodeResource> node_resource)
{
...
fine::ResourcePtr<Exchange10Resource> create_exchange10_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<1, 0>>(data);
    return fine::make_resource<Exchange10Resource>(
...
fine::ResourcePtr<Exchange11Resource> create_exchange11_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<1, 1>>(data);
    return fine::make_resource<Exchange11Resource>(
        std::move(op), problem_resource->data);
}
...
fine::ResourcePtr<Exchange20Resource> create_exchange20_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<2, 0>>(data);
    return fine::make_resource<Exchange20Resource>(
        std::move(op), problem_resource->data);
}
...
fine::ResourcePtr<Exchange21Resource> create_exchange21_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<2, 1>>(data);
    return fine::make_resource<Exchange21Resource>(
        std::move(op), problem_resource->data);
}
...
fine::ResourcePtr<Exchange22Resource> create_exchange22_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<2, 2>>(data);
    return fine::make_resource<Exchange22Resource>(
        std::move(op), problem_resource->data);
}
...
fine::ResourcePtr<Exchange30Resource> create_exchange30_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<3, 0>>(data);
    return fine::make_resource<Exchange30Resource>(
        std::move(op), problem_resource->data);
}
...
fine::ResourcePtr<Exchange31Resource> create_exchange31_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<3, 1>>(data);
    return fine::make_resource<Exchange31Resource>(
        std::move(op), problem_resource->data);
}
...
fine::ResourcePtr<Exchange32Resource> create_exchange32_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<3, 2>>(data);
    return fine::make_resource<Exchange32Resource>(
        std::move(op), problem_resource->data);
}
...
fine::ResourcePtr<Exchange33Resource> create_exchange33_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::Exchange<3, 3>>(data);
    return fine::make_resource<Exchange33Resource>(
        std::move(op), problem_resource->data);
}
...
// Exchange10 apply
fine::Atom exchange10_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange10Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
// Exchange11 apply
fine::Atom exchange11_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange11Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
// Exchange20 apply
fine::Atom exchange20_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange20Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
// Exchange21 apply
fine::Atom exchange21_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange21Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
// Exchange22 apply
fine::Atom exchange22_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange22Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
// Exchange30 apply
fine::Atom exchange30_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange30Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
// Exchange31 apply
fine::Atom exchange31_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange31Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
// Exchange32 apply
fine::Atom exchange32_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange32Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
// Exchange33 apply
fine::Atom exchange33_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<Exchange33Resource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
fine::ResourcePtr<SwapStarResource> create_swap_star_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource,
    double overlap_tolerance)
{
    ...
    return fine::make_resource<SwapStarResource>(
        std::move(op), problem_resource->data);
}
...
fine::ResourcePtr<SwapRoutesResource> create_swap_routes_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::SwapRoutes>(data);
    return fine::make_resource<SwapRoutesResource>(
        std::move(op), problem_resource->data);
}
...
{
    return static_cast<int64_t>(op_resource->op->evaluate(
        route1_resource->route(), route2_resource->route(), evaluator_resource->evaluator));
}

FINE_NIF(swap_star_evaluate_nif, 0);

// SwapStar apply (takes two Routes)
fine::Atom swap_star_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SwapStarResource> op_resource,
    fine::ResourcePtr<SearchRouteResource> route1_resource,
    fine::ResourcePtr<SearchRouteResource> route2_resource)
...
fine::Atom swap_routes_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SwapRoutesResource> op_resource,
    fine::ResourcePtr<SearchRouteResource> route1_resource,
    fine::ResourcePtr<SearchRouteResource> route2_resource)
...
fine::ResourcePtr<SwapTailsResource> create_swap_tails_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    auto& data = *problem_resource->data;
    auto op = std::make_unique<search::SwapTails>(data);
    return fine::make_resource<SwapTailsResource>(
        std::move(op), problem_resource->data);
}
...
// SwapTails apply
fine::Atom swap_tails_apply_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SwapTailsResource> op_resource,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<SearchNodeResource> v_resource)
...
bool relocate_with_depot_supports_nif(
    ...
    fine::ResourcePtr<ProblemDataResource> problem_resource)
{
    return pyvrp::search::supports<pyvrp::search::RelocateWithDepot>(*problem_resource->data);
}

FINE_NIF(relocate_with_depot_supports_nif, 0);

// Helper function to create search route from visits
fine::ResourcePtr<SearchRouteResource> make_search_route_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<ProblemDataResource> problem_resource,
    ...
    int64_t vehicle_type)
{
    ...
    auto route_resource = fine::make_resource<SearchRouteResource>(
        std::make_unique<search::Route>(data, static_cast<size_t>(idx), static_cast<size_t>(vehicle_type)),
        problem_resource->data);

    // Create and append nodes for each client visit
    for (int64_t loc : visits) {
        auto node = std::make_unique<search::Route::Node>(static_cast<size_t>(loc));
...
int64_t insert_cost_nif(
    ...
    fine::ResourcePtr<CostEvaluatorResource> evaluator_resource)
{
    return static_cast<int64_t>(search::insertCost(
        u_resource->node,
        v_resource->node,
        *problem_resource->data,
...
int64_t remove_cost_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<SearchNodeResource> u_resource,
    fine::ResourcePtr<ProblemDataResource> problem_resource,
    fine::ResourcePtr<CostEvaluatorResource> evaluator_resource)
{
    return static_cast<int64_t>(search::removeCost(
        u_resource->node,
        *problem_resource->data,
        evaluator_resource->evaluator));
}
...
int64_t inplace_cost_nif(
    ...
    fine::ResourcePtr<CostEvaluatorResource> evaluator_resource)
{
    return static_cast<int64_t>(search::inplaceCost(
        u_resource->node,
        v_resource->node,
        *problem_resource->data,
        evaluator_resource->evaluator));
...
fine::ResourcePtr<RNGResource> create_rng_from_seed_nif(
    ErlNifEnv* env,
    uint64_t seed)
{
...
fine::Ok<fine::ResourcePtr<RNGResource>> create_rng_from_state_nif(
    ErlNifEnv* env,
    std::vector<uint64_t> state_vec)
{
    ...
    std::array<uint32_t, 4> state = {
        static_cast<uint32_t>(state_vec[0]),
...
std::tuple<fine::ResourcePtr<RNGResource>, uint64_t> rng_call_nif(
    ErlNifEnv* env,
...
std::tuple<fine::ResourcePtr<RNGResource>, double> rng_rand_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<RNGResource> rng_resource)
{
    // Create a new RNG resource with copy of state
...
std::vector<uint64_t> rng_state_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<RNGResource> rng_resource)
{
...
fine::ResourcePtr<DynamicBitsetResource> create_dynamic_bitset_nif(
    ErlNifEnv* env,
    uint64_t num_bits)
{
    return fine::make_resource<DynamicBitsetResource>(static_cast<size_t>(num_bits));
}

FINE_NIF(create_dynamic_bitset_nif, 0);

// Get the size (length) of the bitset
uint64_t dynamic_bitset_len_nif(
    ErlNifEnv* env,
    ...
    bool value)
{
    ...
    auto new_bitset = fine::make_resource<DynamicBitsetResource>(bitset_resource->bitset);
...
fine::ResourcePtr<DynamicBitsetResource> dynamic_bitset_or_nif(
    ErlNifEnv* env,
...
fine::ResourcePtr<DynamicBitsetResource> dynamic_bitset_and_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DynamicBitsetResource> a_resource,
    fine::ResourcePtr<DynamicBitsetResource> b_resource)
{
...
fine::ResourcePtr<DynamicBitsetResource> dynamic_bitset_xor_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DynamicBitsetResource> a_resource,
    fine::ResourcePtr<DynamicBitsetResource> b_resource)
{
...
fine::ResourcePtr<DynamicBitsetResource> dynamic_bitset_not_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DynamicBitsetResource> bitset_resource)
{
...
fine::ResourcePtr<DurationSegmentResource> create_duration_segment_nif(
    ErlNifEnv* env,
    int64_t duration,
    int64_t time_warp,
    int64_t start_early,
...
fine::ResourcePtr<DurationSegmentResource> duration_segment_merge_nif(
    ErlNifEnv* env,
...
// Get the duration
int64_t duration_segment_duration_nif(
    ErlNifEnv* env,
...
int64_t duration_segment_time_warp_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DurationSegmentResource> seg,
...
// Get start_early
int64_t duration_segment_start_early_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DurationSegmentResource> seg)
{
...
// Get start_late
int64_t duration_segment_start_late_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DurationSegmentResource> seg)
{
...
// Get end_early
int64_t duration_segment_end_early_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DurationSegmentResource> seg)
{
...
// Get end_late
int64_t duration_segment_end_late_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DurationSegmentResource> seg)
{
...
// Get slack
int64_t duration_segment_slack_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<DurationSegmentResource> seg)
{
...
fine::ResourcePtr<LoadSegmentResource> create_load_segment_nif(
    ErlNifEnv* env,
    int64_t delivery,
    int64_t pickup,
    int64_t load,
...
fine::ResourcePtr<LoadSegmentResource> load_segment_merge_nif(
    ErlNifEnv* env,
...
fine::ResourcePtr<LoadSegmentResource> load_segment_finalise_nif(
    ErlNifEnv* env,
...
// Get delivery amount
int64_t load_segment_delivery_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<LoadSegmentResource> seg)
{
...
// Get pickup amount
int64_t load_segment_pickup_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<LoadSegmentResource> seg)
{
...
int64_t load_segment_excess_load_nif(
    ErlNifEnv* env,
    fine::ResourcePtr<LoadSegmentResource> seg,
    int64_t capacity)
{
...
struct PerturbationManagerResource {
    ...
    PerturbationManagerResource(size_t min_perturbations, size_t max_perturbations)
...
fine::ResourcePtr<PerturbationManagerResource> create_perturbation_manager_nif(
    [[maybe_unused]] ErlNifEnv* env,
    int64_t min_perturbations,
    int64_t max_perturbations)
{
    if (min_perturbations > max_perturbations) {
        throw std::invalid_argument("min_perturbations must be <= max_perturbations.");
    }
