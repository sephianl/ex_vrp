defmodule ExVrp.MultiTripTest do
  ...
  describe "large depot reload time scenarios" do
    ...
    test "reload time gap respected across shifts with vehicle group" do
      # Two shifts for the same driver, spaced far enough apart to satisfy min_gap
      # Shift 1: [0, 100], shift 2: [900, 1400]
      # With depot_reload_time=800, the natural gap (~900-12=888) exceeds the min_gap
      # Vehicle group enforces the gap constraint at the solver level
      depot_reload_time = 800
      ...
      model =
        ...
        # Shift 2: 900-1400 (gap of ~888 from shift 1 end, > 800)
        |> Model.add_vehicle_type(
          num_available: 1,
          capacity: [100],
          tw_early: 900,
        ...
        # Group both shifts as the same driver with 800 min gap
        |> Model.add_vehicle_group(vehicle_types: [0, 1], min_gap: depot_reload_time)

      {:ok, result} = Solver.solve(model, stop: ExVrp.StoppingCriteria.max_iterations(2000))
      ...
      shift1_end = List.last(shift1_schedule).end_service
      shift2_start = hd(shift2_schedule).start_service
