...
```bash
mix deps.get && mix compile                # Elixir only — see the NIF build gotcha below
mix test                                   # pure Elixir tests
mix test --include nif_required            # includes NIF-dependent tests

# Real-planning benchmarks (this repo, priv/benchmark_data/production/):
mix test test/production_benchmark_test.exs --include production_benchmark        # feasibility gate (long)
mix test test/production_benchmark_test.exs --include production_benchmark_quick  # 5-seed quick check
# A/B objective comparison harness: dev/ab_benchmark/ (Runner + Comparator)
```
...
> you MUST force a local build:
>
> ```bash
> EX_VRP_FORCE_BUILD=1 mix compile                       # in this repo
> EX_VRP_FORCE_BUILD=1 mix deps.compile ex_vrp --force   # from a consumer using this as a path dep (e.g. zelo)
> ```
>
> Symptoms of forgetting: your changes silently have no effect; or — if `@version` was bumped or a
> NIF arity changed — the precompiler tries to download a nonexistent release artifact and **every**
> solve fails to load the NIF (looks like "all benchmarks suddenly fail"). `SANITIZE=1` also forces a
> local build (AddressSanitizer/UBSan).
